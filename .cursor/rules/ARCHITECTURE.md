...
```
.cursor/
├── HOW-TO-USE.md              # Главный гайд по всей папке
├── CONTEXT.md                 # Рабочий файл AI для сохранения контекста
│
├── rules/                     # ⭐ АКТУАЛЬНЫЕ инструкции (24 файла)
...
│
├── rules_arch/                # 🗄️ АРХИВ deprecated (17 файлов)
│   └── *.mdc                  # Старые версии инструкций
│
├── rules_alone/               # 🎯 ОДИНОЧНЫЕ инструкции (4 файла)
│   └── *.mdc                  # Для единичных задач
│
└── template/                  # 📝 ШАБЛОНЫ (4 файла + 3 reference)
    └── _template-*.mdc        # Шаблоны для новых инструкций
```

**В корне проекта:**
```
AGENTS.md                      # Quick Start для AI агентов (стандарт agents.md)
```
...
│   ├── protocol-prepare-prompt.mdc  # ⭐ NEW: Подготовка промта (ПЕРВЫЙ ШАГ)
...
│   ├── standard-agent-quality.mdc  # ⭐ NEW: Метрики успеха, границы, feedback
...
└── 📖 DOCUMENTATION
    ...
    └── RECOMMENDATIONS-rules-improvements.md
```

---

## 🔄 FLOW: КАК ЭТО РАБОТАЕТ (v5.2)
...
| Файл | Назначение |
|------|------------|
| `AGENTS.md` (корень проекта) | Quick Start для AI агентов |
...
1. Скопируй шаблон из `template/_template-alone.mdc`
2. Заполни
3. Положи в `rules_alone/`
4. Обнови `rules_alone/HOW-TO-USE.md`

---

## 📈 ПРЕИМУЩЕСТВА АРХИТЕКТУРЫ v5.2
...
8. **AGENTS.md** — стандартный формат quick start
